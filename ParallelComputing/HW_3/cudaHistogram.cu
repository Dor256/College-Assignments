...
int* cudaHistogramCalc(int *arr, int size) {
   ...
   // Allocate GPU memory for each bucket
    ...
    calcBucketsKernel<<<blocksPerGrid, threadsPerBlock>>>(d_A, size / (threadsPerBlock * blocksPerGrid), bucket, blocksPerGrid, threadsPerBlock);
    ...
    // Call merge kernel
    sumBucketsKernel<<<1, BUCKET_SIZE>>>(temp, bucket, blocksPerGrid, threadsPerBlock);
    err = cudaGetLastError();
    if (err != cudaSuccess) {
